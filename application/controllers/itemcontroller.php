...
class itemcontroller extends CI_Controller
{
    ...
    {
        $this->db->select(
            "items.id, items.itemname, categories.catname, subcategories.subcname, units.unitName, items.price, items.quantity, items.status, items.image",
        );
        $this->db->from("items");
        ...
        $data["units"] = $units;
        $data["items"] = $items;
        $data["validationerrorstor"] = $this->session->flashdata(
            "validationerrorstor",
        );
    ...
    public function get_subcategories()
    {
        ...
        $subcategories = $this->db
            ->get_where("subcategories", ["catname" => $categoryId])
            ->result();
    ...
    public function edit($id)
    {
        ...
        if ($item->status == 1) {
            $stutusd = ["value" => 1, "name" => "active"];
        } else {
            $stutusd = ["value" => 0, "name" => "Inactive"];
        }
    ...
    public function update_item()
    {
        ...
        if ($this->input->post("myImage") == "hi") {
            ...
            if (!$this->upload->do_upload("image")) {
                ...
                $this->session->set_flashdata(
                    "success",
                    "please try again update image formate gif|jpg|png ",
                );
